...
- [x] **Replace the full contents of `src/types.ts`** with the following:
...
- [x] **Run typecheck** — should pass with no errors (no callers use new types yet):
...
- [x] **Write tests** in `src/DropSign.test.ts` — add a new `describe` block at the top of the file (before the existing `describe('DropSign.init', ...)` block):
...
- [x] **Replace the full contents of `src/styles.ts`** with the following (key changes: remove hardcoded `bottom`/`right` from `.ds-button`, add viewport class, press effect, inline trigger styles, mobile media queries):
...
- [x] **Run tests** (no new tests for styles — observable behavior covered elsewhere):
...
- [x] **Replace the full contents of `src/signature-pad.ts`**:
...
- [x] **Replace the full contents of `src/placement.ts`**:
...
- [x] **Replace the full contents of `src/DropSign.ts`**:
...
- [x] **Replace the contents of `src/index.ts`**:
...
- [x] **Add the following `describe` block** inside the existing file, after the `describe('DropSign.init', ...)` block and before `describe('captureResult', ...)`:
...
- [x] **Replace `examples/basic/index.html`** with the following:
...
- [x] **Replace `examples/basic/src/main.ts`** with the following:
...
- [x] **Start the demo and verify it works** (manual check — you cannot automate this step):
...
- [x] **Replace the full contents of `README.md`** with the following:
...
- [x] **Verify dist exports** — confirm the new types are in the declaration file:
...
- [x] **Commit any fixes**, then tag the work:
